# 性能优化四板斧：Packed Buffer、缓存与闪烁消除

> 上一篇《从 React 到终端字符：ink 渲染管线全景》追溯了 ink 从 `setState` 到终端屏幕的完整渲染流程。那条管线在功能上是完整的，但每帧创建几千个 JavaScript 对象、全量重绘未变组件、逐 cell 全量对比——这些问题让终端渲染无法达到流畅的帧率。

本篇聚焦 fork 在渲染层的四项核心优化：用 Packed Int32Array 消除 GC 压力、用三池（CharPool / StylePool / HyperlinkPool）让 cell 对比变成整型比较、用 Blit Cache 跳过未变组件的渲染、用 Damage Tracking 把 diff 范围缩到最小。最后讨论闪烁问题，以及 fork 如何用四层方案从根本上消除它。
...
四板斧解决了渲染的效率问题——内存紧凑、缓存复用、diff 精准、无闪烁。但终端 UI 不只是"画得快"，还要能交互。用户要点击组件、选择文本、滚动内容、触发快捷键。下一篇《终端里的浏览器体验：事件、选择与虚拟滚动》会展开 fork 为终端实现的这些交互能力。

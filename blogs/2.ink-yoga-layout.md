# 【02】Code Agent 从零到一：Packed Buffer、缓存与闪烁消除
...
> 本篇介绍的优化全部是 **fork 在开源 ink 基础上新增的特性**。开源 ink 用的是对象数组 + 全量 diff + 逐帧清屏重绘；fork 把底层数据结构、缓存策略、diff 算法全部重写了。阅读时可以对照上一篇的渲染管线，看 fork 在管线的哪些环节插入了优化。
...
把数据流画出来，三个池子和 Packed Buffer 的协作是这样的：

```
setCellAt(x, y, { char: 'H', style: [bold, red], link: 'http://...' })
    │
    ├─ char  →  CharPool.intern('H')       → charId = 72
    ├─ style →  StylePool.intern([31, 1])  → styleId = 42
    ├─ link  →  HyperlinkPool.intern('...') → hyperlinkId = 3
    │
    └─ 打包  →  cells[(y * cols + x) * 2]     = 72
              →  cells[(y * cols + x) * 2 + 1] = (42 << 17) | (3 << 2) | 1
                                                   ▲             ▲         ▲
                                               StylePool ID   链接 ID   宽度
```

三个池子把 cell 的语义信息（字符、样式、链接）全部压成了整数。对比两个 cell 是否相同，不再是比较 3 个字符串 + 1 个数组，而是比较 2 个 Int32。两行 CPU 指令，一条比较 word0，一条比较 word1。这就是为什么 diff 能快到一个数量级以上。

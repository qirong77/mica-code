...
> 本篇介绍的四套交互系统全部是 **fork 在开源 ink 基础上新增的特性**。开源 ink 只做全局 stdin 监听，把所有输入当作原始字节流转发给 React reconciler——没有事件分发、没有选区管理、没有滚动容器。fork 在这些空白领域逐个补上了完整的能力。

## 过渡：没有事件系统的时候，交互长什么样？

在浏览器里，你写一个 `onClick` 处理器，点按钮就触发——因为浏览器知道按钮在哪、鼠标在哪、事件应该发给谁。

在终端里，你没有这些。你只有一个 stdin 字节流。用户按了一个键，你收到一个字节序列。你要自己判断"这个按键应该发给哪个组件"、"当前鼠标下面是什么"、"用户是在拖拽选择还是点击按钮"。

开源 ink 的做法就是：把所有 stdin 输入都转发给 React reconciler，由 React 的合成事件系统处理。这意味着：

- **无法知道鼠标在哪**——终端不默认报告鼠标坐标
- **无法区分粘贴和键盘输入**——都是 stdin 字节流
- **无法感知焦点变化**——用户在终端里还是切出去了？不知道
- **快捷键冲突**——所有按键都是全局的，无法根据当前焦点组件切换行为

fork 从事件分发、选区管理、滚动容器三个维度补上了这些缺失。下面的第六章到第八章，就是 fork 在这个"无交互的字符网格"上搭建的完整交互层。

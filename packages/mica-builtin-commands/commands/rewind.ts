...
import type { OneLineItemCell } from '@packages/mica-ui/index.js';
...
type RewindPreviewOk = Extract<RewindPreviewResult, { ok: true }>;

const PANEL_ID = 'rewind-panel';
const MAX_VISIBLE_FILES = 14;
const element = React.createElement;

export function createRewindCommand(services: CommandRuntimeServices): BuiltInCommandItem {
  return {
    name: 'rewind',
...
function showRewindPanel(checkpoints: RewindCheckpointSummary[], services: CommandRuntimeServices) {
  const phase = atom<'checkpoint' | 'scope'>('checkpoint');
  const checkpointIndex = atom(0);
  const modeIndex = atom(0);
  const preview = atom<RewindPreviewOk | null>(null);
  ...
  function refreshStalePreview(id: string) {
  ...
  function ScopePanel({ current }: { current: RewindPreviewOk }) {
...
function renderRewindCheckpointItem(item: { description?: unknown; label: unknown }, isSelected: boolean) {
  return element(micaUi.OneLineItem, {
    cells: rewindCheckpointCells(
      { description: item.description === undefined ? '' : String(item.description), label: item.label },
      isSelected,
    ),
  });
}

export function rewindCheckpointCells(
  item: { description: string; label: unknown },
  isSelected: boolean,
): OneLineItemCell[] {
  return [
    {
    ...
    {
      key: 'label',
      content: item.label === undefined ? '' : String(item.label),
      flexGrow: 1,
      flexShrink: 1,
      minWidth: 0,
      color: isSelected ? micaUi.theme.colors.accent : undefined,
...
function renderFileImpact(current: RewindPreviewOk, visibleFiles: RewindFileChange[], hiddenCount: number) {
...
function formatSuccessNotice(result: RewindApplyResult) {
...
function isRunningStatus(status: { type: string }) {

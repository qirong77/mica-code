...
- 注册产品内置命令，例如 `/model`、`/provider`、`/resume`、`/mcp`、`/skills`、`/log`、`/status`、`/compact`、`/agents`、`/fork`、`/rewind`、`/commit`、`/clear`。
- 提供命令所需的服务类型与注入入口。
...
- `compact.ts`：上下文压缩命令。
- `logs.tsx`、`logExport.ts`、`status.tsx`：日志查看、日志导出与状态查看命令；`/log export` 复用导出逻辑。

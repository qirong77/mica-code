...
- 注册产品内置命令，例如 `/model`、`/provider`、`/resume`、`/mcp`、`/skills`、`/log`、`/status`、`/task`、`/context`、`/doctor`、`/compact`、`/agents`、`/fork`、`/rewind`、`/review`、`/commit`、`/clear`。
- 提供命令所需的服务类型与注入入口。
...
- `compact.ts`：上下文压缩命令。
- `log.tsx`、`status.tsx`、`task.tsx`、`context.tsx`、`doctor.tsx`：日志查看、日志导出、状态查看、后台任务查看、上下文总览与环境诊断命令。

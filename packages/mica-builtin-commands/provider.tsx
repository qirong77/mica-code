...
export function createProviderCommand(agent: CommandAgent, services: CommandRuntimeServices) {
  return {
    name: 'provider',
    ...
    action: () => {
      ...
      showSelectCommand({
        ...
        onSelect: (providerId) => {
          ...
          micaLogger.logRuntime('plugin.provider', 'selected', { from: micaConfig.get().provider, to: providerId });
          ...
          if (provider?.get_model_url && !provider.models?.length) {
            ...
            void micaConfig.loadProviderModels(providerId).then(() => {
              ...
              micaLogger.logRuntime('plugin.provider', 'models:load:error', {
                provider: providerId,

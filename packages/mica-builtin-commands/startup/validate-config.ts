...
type ValidateConfigContext = {
  paths?: { config?: string };
  logger?: { info?(event: string, data?: unknown): void; warn?(event: string, data?: unknown): void };
};

/**
 * Mica startup validation plugin. The startup bootstrap applies defaults before
 ...
 */
export default function setupValidateConfig(ctx: ValidateConfigContext): void {
 ...
 */
export function applyConfigDefaultsToFile(configPath: string): ConfigValidationResult {
...
export function validateConfigText(content: string, configPath = 'config.json'): ConfigValidationResult {
  const parsed = parseConfigText(content, configPath);
  if (!parsed.ok) return parsed;
...
export function validateConfig(input: unknown): ConfigValidationResult {
  ...
  const add = (severity: ConfigIssue['severity'], code: string, path: string, message: string, suggestion?: string) => {
  ...
  (config.providers as unknown[]).forEach((provider, index) => {
    ...
    if (!PROVIDER_PROTOCOLS.includes(provider.protocol as string)) {
      add(
        'error',
...
export function assertValidConfig(result: ConfigValidationResult, configPath = 'config.json'): ConfigValidationResult {
  if (!result.ok) throw new ConfigValidationError(result.issues, configPath);
  return result;
}

export function formatConfigValidationIssues(issues: ConfigIssue[], configPath = 'config.json'): string {
...
function applyConfigDefaults(input: unknown): { config: unknown; changed: boolean; changes: ConfigChange[] } {
  if (!isRecord(input) || !Array.isArray(input.providers)) {
    return { config: input, changed: false, changes: [] };
  }

  const changes: ConfigChange[] = [];
  const providers = (input.providers as unknown[]).map((provider, index) => {
...
function validateMcpServers(mcpServers: unknown, add: (severity: ConfigIssue['severity'], code: string, path: string, message: string, suggestion?: string) => void): void {
...
function parseConfigText(content: string, configPath: string): ConfigValidationResult {
  try {
...
function validationResult(config: unknown, changed: boolean, changes: ConfigChange[], issues: ConfigIssue[]): ConfigValidationResult {
  return {
...
function emptyFileResult(): ConfigValidationResult {
  return { ok: true, config: null, changed: false, changes: [], issues: [] };
}

function writeConfigFile(configPath: string, config: unknown): void {

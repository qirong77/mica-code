...
setSelectEmitter((item: MicaUiDropdownItem) => { for (const h of _selectHandlers) h(item); });

export const DropDownUI = {
  renderFn: DropDownSelect,
  onSelect: (cb: (item: MicaUiDropdownItem) => void) => {
    _selectHandlers.push(cb);
    return () => { const idx = _selectHandlers.indexOf(cb); if (idx !== -1) _selectHandlers.splice(idx, 1); };
  },

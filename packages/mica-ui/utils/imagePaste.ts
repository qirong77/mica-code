...
export function getImageFromClipboard(): ImageData | null {
  ...
  try {
    ...
    execFileSync('osascript', ['-e', `set png_data to (the clipboard as «class PNGf»)\nset fp to open for access POSIX file "${filePath}" with write permission\nwrite png_data to fp\nclose access fp`], { stdio: 'ignore', timeout: 5000 });
...
export function parseImageRefs(text: string): string | import('../types.js').MicaUiContentBlockParam[] {
  ...
  while ((match = IMAGE_REF_RE.exec(text)) !== null) {
    const [full, imgPath] = match, idx = match.index;
    ...
    try {
      ...
      blocks.push({ type: 'text', text: full }, { type: 'image', source: { type: 'base64', media_type: mediaType, data: buffer.toString('base64') } });
    } catch { blocks.push({ type: 'text', text: full }); }
    lastIndex = idx + full.length;
  }
  if (lastIndex < text.length) blocks.push({ type: 'text', text: text.slice(lastIndex) });
  if (blocks.length === 0) return text;
  if (blocks.length === 1 && blocks[0]!.type === 'text') return (blocks[0] as import('../types.js').MicaUiTextBlock).text;
  return blocks;
}
...
function cleanupTempDir(): void {
  try {
    ...
    const entries = readdirSync(TEMP_IMAGE_DIR).map((name) => {
      ...
      try { return { name, path: p, mtime: statSync(p).mtimeMs }; } catch { return null; }
    }).filter((e): e is NonNullable<typeof e> => e !== null).sort((a, b) => b.mtime - a.mtime);
    if (entries.length > 100) { for (const f of entries.slice(100)) rmSync(f.path, { force: true }); }
  } catch { /* ignore */ }
}
cleanupTempDir();

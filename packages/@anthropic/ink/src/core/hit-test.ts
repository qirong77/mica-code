...
export function dispatchMouseAction(
  ...
  let target: DOMElement | undefined = targetOverride ?? hitTest(root, col, row) ?? undefined;
  if (!target) return null;

  const propName = type === 'mousedown' ? 'onMouseDown' : type === 'mouseup' ? 'onMouseUp' : 'onMouseDrag';

  const event = new MouseActionEvent(type, col, row, button);
  let handledBy: DOMElement | null = null;

  while (target) {
    const handler = target._eventHandlers?.[propName] as ((event: MouseActionEvent) => void) | undefined;
    if (handler) {
      handledBy ??= target;

...
const preferredPort = process.env.MICA_CONFIG_WEB_PORT?.trim() || '5177';
...
async function waitForReady(expectedToken, timeoutMs = 20_000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if (existsSync(statePath)) {
      try {
        const state = JSON.parse(readFileSync(statePath, 'utf-8'));
        if (state?.token === expectedToken && state?.port) {
          const response = await fetch(
            `http://127.0.0.1:${state.port}/api/ping?token=${encodeURIComponent(expectedToken)}`,
...
const child = spawn(process.execPath, ['src/index.ts', '--config-web-worker', token], {
  cwd: process.cwd(),
  env: {
    ...process.env,
...
try {
  const state = await waitForReady(token);

...
try {
  ...
  if (!rcFile) {
    console.log(
      `Warning: could not find shell rc file to add ${installDir} to PATH.`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `Warning: install skipped (${error instanceof Error ? error.message : String(error)}).`,
  );
  console.log(
    `You can copy ${outFile} to ${installDir} manually, or set MICA_INSTALL_DIR to a writable directory.`,
  );
}

...
const INIT_PROMPT = `请分析当前代码库并创建或更新 MICA.md 文件。MICA.md 会在每次 Mica Code 会话启动时注入 system prompt，因此内容必须简洁——只包含 AI 不知道会犯错的指令，同时包含足够的项目上下文让 AI 能高效工作。

你需要读取：
- package.json（scripts、workspaces、依赖）
- tsconfig.json（paths 别名、编译器选项）
- README.md / 现有的 MICA.md
- 构建脚本、测试/lint 配置
...
- MICA.md 在启动时自动注入 system prompt，编辑此文件直接影响 agent 行为
\`\`\``;

...
describe('RewindCheckpointManager conversation fallback', () => {
  it('rebuilds resumed turns and removes the selected turn including tool messages', () => {
    ...
    expect(result.inputText).toBe('second request');
  ...
  it('keeps a live checkpoint at the state before its selected input', () => {
    const manager = new RewindCheckpointManager();
    let currentSnapshot = makeSnapshot([]);
    const agent = makeAgent(
      () => currentSnapshot,

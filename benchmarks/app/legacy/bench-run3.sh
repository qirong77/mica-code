...
cell_running() {
  # cell.sh and cell2.sh both count: a matrix may have in-flight cells launched
  # by either (this runner exists because cell.sh could not be edited in place).
  pgrep -f "cell2?\.sh ${TAG} ${1} ${2}$" >/dev/null 2>&1
}
...
while [ "${#PENDING[@]}" -gt 0 ]; do
  running=$(pgrep -f "cell2?\.sh ${TAG} " 2>/dev/null | wc -l | tr -d ' ')

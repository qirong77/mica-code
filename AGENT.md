...
- 软键盘避让：键盘弹起时 iOS Safari 与 Android Chrome 都只缩 visual viewport、不缩布局视口，`dvh` 只跟随地址栏。`hooks.js` 的 `useVisualViewportHeight` 把可见高度写进 `--vvh`（按 `scale` 还原以兼容双指缩放，只在整数值变化时写，避免亚像素抖动让终端反复重排），窄屏根容器高度是 `var(--vvh, 100dvh)`——**不要改回固定 `100dvh`**，否则对话输入条与终端功能键栏会被留在键盘下面，既看不见也点不到。
...
- 会话行的终端标记：右侧面板终端不在 `nodes` 里，创建时把当前会话 id 记进 `rightTerms` 条目（`App.jsx` 的 `createRightTerm`），`sessionsWithRunningTerminal` 再用 notify 状态里的 `processRunning`（`serializeStates` 必须同时透出 `agentRunning`/`processRunning`，`running` 是二者合并值、分不出终端）映射回左侧会话行，行首显示高亮终端图标。`processRunning` 由 `terminals.js` 的前台进程轮询（≥1.2s 才算，仅 macOS/Linux）经 notify server 上报。该标记与 Mica turn 的呼吸绿点、未读圆点、异常终止红点共用 `SessionTree.jsx` 行首那个固定 `w-4` 的状态位（`RowLeading`）：终端图标 > running 绿点（`chat-dot-running`，只做透明度呼吸，不缩放也不给整行标题变色）> 未读圆点 / 红点；显示终端图标且同时带圆点时把圆点绝对定位叠在图标右上角，**不要另起一个 flex 兄弟节点**，否则这一行会比相邻行多占「图标宽 + gap」（22px）、标题缩进对不齐。
- 会话行的红点（`liveSessionRowState` 的 `error` 态，`bg-danger` 常亮、不加动画）：表示这一轮**没有正常运行完成**，是会话状态不是未读徽标——读过不消失，只有后续某轮 completed 才清掉。判定有两条来源：本次进程内的 notify `lastType === 'turn.error'`，以及 host 在 `stats:list-sessions` 行上补的 `interrupted`（`src/host/session-lease.js` 的 `isInterruptedSession`：`turnState === 'error'`，或 `turnState === 'running'` 却已无人持有 `sessions/.turn-locks/<id>.lock`，即进程被中断/崩溃留下的残留）。**`running` 必须先探测 turn lease 的持有者 pid 存活**（`createTurnLeaseProbe`，与 `packages/mica-session/sessionStore.ts` 的孤儿锁回收同源，ESRCH 才算死）再判红——另一个终端里的 TUI 可能正在跑同一个会话，误标成异常是错的；用户主动中断的 `aborted` 也**不算**异常。lease 存活不能进 `stats-scanner` 的文件签名缓存（文件没变但持有者可能已消失），所以 `scanMeta()` 每次按次探测，但结果不变时必须把原数组/行对象引用交回去（侧栏靠引用相等跳过重渲染）。

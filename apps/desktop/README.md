...
**定时任务** is the runtime's own list, not a page-level one: `src/host/scheduled-tasks.js` holds the pure model (normalize / create / quota / due selection, unit-tested) and `src/host/scheduled-runner.js` the host (one re-armed timer, `userData/scheduled-tasks.json`, the `schedule:*` IPC), so a task keeps firing after the tab is closed, the page reloaded, or a phone opens the same runtime — the sidebar section and the composer's clock panel read that one list, and every change is broadcast as `schedule:changed`. A task means "send this message into this session every N minutes, M times" (an empty count, i.e. `0`, is unlimited — the form defaults to 10 — and the interval is capped at a week so the timer can never overshoot). Sending reuses the chat host's own `startRun`, so a session that is already busy is *skipped* instead of queued: the run counter stays put, the next attempt moves one interval out, and the reason lands in the row's tooltip. It claims the tab already showing that session whenever the workspace has one (the user watches the turn live and can abort it) and only falls back to a `scheduled:<taskId>` run when nothing is open. A row opens the session it belongs to; its context menu (long-press on touch) offers 立即发送一次 / 暂停 / 继续 / 删除定时任务, and the countdown lives in the tooltip because the tree never ticks per second (that would re-render the whole sidebar for a clock). Deleting a conversation drops its tasks (`stats:delete-session` → `dropTasksForSession`), and a session deleted behind the runtime's back makes a firing task skip rather than spawn a doomed host.
...
- `GET /api/events` is a Server-Sent Events stream carrying every push channel
  ...
  `ui-state:changed`, `schedule:changed`).
...
- `MICA_DESKTOP_USER_DATA` (the container passes `app.getPath('userData')`) and the shim default
  ...
  `session-pins.json`, `session-projects.json`, `session-sort.json` and `scheduled-tasks.json` are
  shared between the desktop app and a standalone runtime.
...
Scheduled tasks are host-owned state that is deliberately *not* UI state: they outlive the page, so they are persisted to `userData/scheduled-tasks.json` and scheduled by the runtime (`scheduled-runner.js`) instead of being rebuilt from a page-side `setInterval` — see the 定时任务 paragraph above.

...
- **切换工作目录**：发送按钮左侧展示当前会话 cwd，点击弹出列表（当前 + 该机器最近使用过的目录，可输入任意路径）；切换通过 `update_cwd` 指令即时生效，会话被占用时会显示失败原因
- **会话列表**：标题、状态徽标（运行中/已完成/已中止/出错）、相对时间，按更新时间排序
- **会话详情**：顶部信息栏精简为一行（机器 · `model_effort` · 上下文占用 `N.NK (cached %, ctx %)`，连接状态用圆点表示，断开时显示文字）；消息流（用户 / 助手 / 工具调用折叠卡片 / thinking / notice）；底部输入框（Enter 发送，运行中可中止）
...
- 上下文占用摘要从 `snapshot.lastUsage`（detail 默认保留）+ `snapshot.contextWindowSize` 计算，SSE `usage` 事件实时更新

## 构建

```bash
bun run build:sync-web   # -> dist，由 mica-sync-server --web-dir 指向
```

...
export async function startSyncServer(options: SyncServerOptions): Promise<RunningSyncServer> {
  ...
  const commands = new Map<string, DaemonCommand[]>();
  const pollWaiters = new Map<string, Array<{ resolve: (commands: DaemonCommand[]) => void; timer: NodeJS.Timeout }>>();
  const webDir = options.webDir ? resolve(options.webDir) : undefined;

  function enqueueCommand(machineId: string, command: DaemonCommand): void {
  ...
  function takeCommands(machineId: string): DaemonCommand[] {
    const queue = commands.get(machineId) ?? [];
...
type HandlerContext = {
  store: SyncStore;
  hub: EventHub;
  enqueueCommand: (machineId: string, command: DaemonCommand) => void;
  takeCommands: (machineId: string) => DaemonCommand[];
  pollWaiters: Map<string, Array<{ resolve: (commands: DaemonCommand[]) => void; timer: NodeJS.Timeout }>>;
...
async function handleRequest(
  ...
  if (pathname === '/daemon/beat' && method === 'POST') {
    ...
    const body = await readJsonBody(request);
    const active = isRecord(body.active) ? body.active : {};
  ...
  if (pathname === '/daemon/poll' && method === 'POST') {
    ...
    await new Promise<void>((resolvePoll) => {
      ...
      const waiter = {
        resolve: (commands: DaemonCommand[]) => {
          clearTimeout(timer);
          writeJson(response, 200, { commands });
          resolvePoll();
        },
...
function removeWaiter(
  ctx: HandlerContext,
  machineId: string,
  target: { resolve: (commands: DaemonCommand[]) => void; timer: NodeJS.Timeout },
): void {

...
describe('create session endpoint', () => {
  ...
  it('accumulates commands while the daemon is offline and delivers them in order on the next poll', async () => {
    const registered = await request('/daemon/register', {
      method: 'POST',
      body: { name: 'host', hostname: 'host.local', platform: 'darwin', version: '1' },
    });
    ...
    // Two web actions while the daemon has no poll connection open.
    await request(`/api/machines/${machineId}/sessions`, { method: 'POST', body: { text: 'first' } });
    await request(`/api/machines/${machineId}/sessions`, { method: 'POST', body: { text: 'second' } });

    const polled = await request('/daemon/poll', { method: 'POST', body: {}, machineId });
    const commands = polled.body.commands as Array<Record<string, unknown>>;
    expect(commands).toHaveLength(2);
    expect(commands.map((c) => c.prompt)).toEqual(['first', 'second']);
  });
});

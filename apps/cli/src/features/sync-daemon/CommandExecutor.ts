...
export class CommandExecutor {
  ...
  /** Same-poll aborts keyed to the exact run command they cancel. */
  private readonly pendingAborts = new Map<string, string>();
  private busy = false;
  ...
  abort(sessionId?: string, pendingCommandId?: string): void {
    if (!sessionId) {
      for (const value of this.hosts.values()) value.executor.abort();
      return;
    }
    // If this session's turn is actively running, abort it now. A not-yet-started
    // run is cancelled only when the poll loop identifies its exact command id.
    if (sessionId === this.currentSessionId && this.hosts.has(sessionId)) {
      this.hosts.get(sessionId)!.executor.abort();
      return;
    }
    if (pendingCommandId) this.pendingAborts.set(sessionId, pendingCommandId);
  }
  ...
  async execute(command: DaemonCommand): Promise<void> {
    ...
    const abortBeforeStart = this.pendingAborts.get(command.sessionId) === command.id;
    if (abortBeforeStart) this.pendingAborts.delete(command.sessionId);

    const store = micaSession.createStore();
    const session = store.load(command.sessionId);
    if (!session) {
      if (command.type === 'create') {
        await this.createSession(command.sessionId, command.prompt, command.cwd, abortBeforeStart);
        return;
      }
    ...
    try {
      ...
      if (abortBeforeStart) {
        this.emit(command.sessionId, [{ type: 'turn', state: 'aborted' }]);
        this.releaseBusy(command.sessionId);
        return;
      }
